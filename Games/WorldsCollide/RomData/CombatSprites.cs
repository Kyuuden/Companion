...
public class CombatSprites : IDisposable
{
    ...
    public CombatSprites(IMemorySpace memorySpace)
    {
        ...
        foreach (Monster monster in Enum.GetValues(typeof(Monster)))
        {
            ...
            _monsterSprites[monster] = new CombatSprite((int)monster, template, tileData, paletteData, template.LargeStencil ? largeStencils : smallStencils);
        }

        foreach (Boss boss in Enum.GetValues(typeof(Boss)))
        {
            var template = templates[(int)boss];
            _bossSprites[boss] = new CombatSprite((int)boss, template, tileData, paletteData, template.LargeStencil ? largeStencils : smallStencils);
        }

        foreach (Esper esper in Enum.GetValues(typeof(Esper)))
        {
            var template = templates[(int)esper];
            _esperSprites[esper] = new CombatSprite((int)esper, template, tileData, paletteData, template.LargeStencil ? largeStencils : smallStencils);
        }
    }
