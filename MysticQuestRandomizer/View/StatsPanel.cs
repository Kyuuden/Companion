...
public abstract class TextStatisticControl<T> : StatisticControl<T, Seed> where T : struct
{
    ...
    public TextStatisticControl(Seed seed, PanelSettings settings)
        : base(seed, settings)
    {
        MinimumSize = new System.Drawing.Size(56, 16);
    }

    protected override System.Drawing.Image Render()
    {
        var icon = GetIcon();
        var text = Game.Font.RenderText(GetStatText());
        var combined = PaletteExtensions.Combine(icon.Palette!, text.Palette!);
        var data = BitmapDataFactory.CreateBitmapData(new System.Drawing.Size(56, 16), KnownPixelFormat.Format8bppIndexed, combined);

        icon.DrawInto(data, new System.Drawing.Rectangle(0, 0, 16, 16), KGySoft.Drawing.ScalingMode.NearestNeighbor);
        text.DrawInto(data, new System.Drawing.Rectangle(16, 4, 40, 8), KGySoft.Drawing.ScalingMode.NearestNeighbor);
